...
class Subscriber(Node):
    ...
    def _assemble_and_process_content(self, content_name):
        ...
        print(f"[OK] Subscriber {self.name} successfully assembled complete content for {content_name} ({len(assembled_content)} bytes)")
...
def run_simulation(routers, publishers, subscribers, policy, iterations, model=None):
    ...
    for iteration in range(iterations):
        ...
        if active_subscribers: # Ensure there is at least one active subscriber
            ...
            print(f"[SEARCH] {subscriber.name} requesting {content_to_request}")
    ...
    for router in routers:
        if hasattr(router, "save_multipath_performance"):
            if hasattr(router, 'multipath_performance_data') and router.multipath_performance_data:
                ...
                print(f"[DEBUG] [OK] Saved multipath CSV with {len(router.multipath_performance_data)} records")
                multipath_data_found = True
            else:
                print(f"[DEBUG] Router {router.name} has no multipath data")

    if not multipath_data_found:
        print(f"[DEBUG] [WARN] No multipath data collected - multipath mode may not have been triggered")
...
def main():
    ...
    if random_forest_model:
        ...
        print("[WARN]️ Random Forest model not available - using simplified prediction")
    ...
    for policy in policies:
        ...
        print(f"\n[SETUP] Running chunked transmission simulation for {policy} policy...")
