...
class Router(Node):
    ...
    def _discover_paths_to_publisher(self, content_name):
        """
        Discover multiple distinct paths to publisher using the mesh topology.
        Uses the stored connections to find alternative routes.
        Returns up to 5 different path options for multi-path transmission.
        """
        paths = []

        # Method 1: Use stored connections (preferred for multi-path topology)
        if hasattr(self, 'connections') and self.connections:
            # Get all routers that this router can reach
            for next_hop in self.connections:
                if isinstance(next_hop, Router):
                    paths.append(next_hop)

        # Method 2: Extract unique next-hops from FIB
        unique_next_hops = set()
        for dest, next_hop in self.fib.items():
            if isinstance(next_hop, Router):
                unique_next_hops.add(next_hop)

        # Combine both methods
        for router in unique_next_hops:
            if router not in paths:
                paths.append(router)

        # Limit to top 5 paths for efficiency (can be adjusted)
        max_paths = 5
        selected_paths = paths[:max_paths]

        if len(selected_paths) > 1:
            self.log_event(f"[MULTIPATH] Discovered {len(selected_paths)} distinct paths for {content_name}")
            for i, path_router in enumerate(selected_paths):
                self.log_event(f"[MULTIPATH]   Path {i+1}: via {path_router.name}")
...
def setup_network():
    """
    Set up a multi-path network topology with multiple distinct routes.
    Creates a mesh-like structure with redundant paths between subscribers and publishers.
    """
    ...
    # Get the number of routers (recommend at least 6 for good multi-path)
    num_routers = get_valid_integer("Enter the number of routers (recommended: 6-10): ")
    if num_routers < 6:
        print(f"⚠ Warning: {num_routers} routers may not provide enough path diversity.")
        print("   Recommended: 6-10 routers for optimal multi-path performance.")
    ...
    # Each subscriber connects to a different edge router
    for i, subscriber in enumerate(subscribers):
        router_index = i % len(routers)
        subscriber.connected_router = routers[router_index]
        print(f"  ✓ {subscriber.name} -> {routers[router_index].name}")

    # Initialize the content ID manager
    ContentIDManager.initialize_index(publishers)

    print(f"\n📡 Creating multi-path network topology...")
    print(f"   Routers: {num_routers}")
    print(f"   Publishers: 2 (cats, dogs)")
    print(f"   Subscribers: {num_subscribers}")

    # ==================================================================
    # MULTI-PATH TOPOLOGY CREATION
    # ==================================================================
    # Strategy: Create a mesh-like topology with multiple distinct paths
    # from edge routers (where subscribers connect) to core routers 
    # (that connect to publishers)

    # Divide routers into layers for multi-path routing:
    # - Edge routers (first 1/3): Where subscribers connect
    # - Middle routers (middle 1/3): Provide path diversity
    # - Core routers (last 1/3): Connect to publishers

    third = max(1, num_routers // 3)
    edge_routers = routers[:third]
    middle_routers = routers[third:2*third] if num_routers > 3 else []
    core_routers = routers[2*third:] if num_routers > 3 else routers[third:]

    if not core_routers:  # Fallback for very small networks
        core_routers = [routers[-1]]

    print(f"\n   Network layers:")
    print(f"   - Edge routers: {[r.name for r in edge_routers]}")
    print(f"   - Middle routers: {[r.name for r in middle_routers]}")
    print(f"   - Core routers: {[r.name for r in core_routers]}")

    # Track connections for debugging
    connection_count = 0

    # Path 1: Edge -> Middle (multiple paths per edge router)
    for edge_router in edge_routers:
        if middle_routers:
            # Each edge router connects to ALL middle routers (full mesh)
            for middle_router in middle_routers:
                edge_router.fib.update({f"cat_image{j}.jpg": middle_router for j in range(1, 51)})
                edge_router.fib.update({f"dog_image{j}.jpg": middle_router for j in range(1, 51)})
                connection_count += 1
        else:
            # Direct connection to core if no middle layer
            for core_router in core_routers:
                edge_router.fib.update({f"cat_image{j}.jpg": core_router for j in range(1, 51)})
                edge_router.fib.update({f"dog_image{j}.jpg": core_router for j in range(1, 51)})
                connection_count += 1

    # Path 2: Middle -> Core (full mesh connectivity)
    for middle_router in middle_routers:
        # Each middle router connects to ALL core routers
        for core_router in core_routers:
            middle_router.fib.update({f"cat_image{j}.jpg": core_router for j in range(1, 51)})
            middle_router.fib.update({f"dog_image{j}.jpg": core_router for j in range(1, 51)})
            connection_count += 1

    # Path 3: Core -> Publishers (all core routers connect to both publishers)
    for core_router in core_routers:
        core_router.fib.update({f"cat_image{j}.jpg": publisher1 for j in range(1, 51)})
        core_router.fib.update({f"dog_image{j}.jpg": publisher2 for j in range(1, 51)})
        connection_count += 1

    # Additional cross-layer connections for even more path diversity
    # Edge routers can also reach core routers directly (bypass middle)
    for edge_router in edge_routers:
        for core_router in core_routers:
            # Add direct paths as alternatives
            edge_router.fib.update({f"cat_image{j}.jpg": core_router for j in range(1, 51)})
            edge_router.fib.update({f"dog_image{j}.jpg": core_router for j in range(1, 51)})
            connection_count += 1

    # Store router connections for path discovery
    for router in routers:
        router.connections = []
        for dest, next_hop in router.fib.items():
            if isinstance(next_hop, Router) and next_hop not in router.connections:
                router.connections.append(next_hop)

    print(f"\n   ✓ Created {connection_count} router connections")
    print(f"   ✓ Multiple distinct paths available for Algorithm 1")

    # Calculate and display path diversity
    sample_router = edge_routers[0]
    unique_next_hops = set()
    for dest, next_hop in sample_router.fib.items():
        if isinstance(next_hop, Router):
            unique_next_hops.add(next_hop.name)

    print(f"   ✓ Example: {sample_router.name} has {len(unique_next_hops)} distinct next-hop options")
    print(f"     Next hops: {list(unique_next_hops)[:5]}{'...' if len(unique_next_hops) > 5 else ''}")

    # Save the new network setup
    save_network(routers, publishers, subscribers)

    print("\n✅ Multi-path network setup created and saved.")
    return routers, publishers, subscribers
...
def run_simulation(routers, publishers, subscribers, policy, iterations, model=None):
    ...
    for iteration in range(iterations):
        ...
        # Calculate cumulative latency (sum of cache access times)
        total_latency = sum(router.total_cache_access_time for router in routers)
        ...
        if (iteration + 1) % 10 == 0:
            print(f"[Iter {iteration+1}] Clients={len(active_subscribers)}, "
                  f"Requests={total_requests}, Hit={avg_cache_hit:.1f}%, Latency={avg_latency:.4f}")
    ...
    print(f"\n[DEBUG] Checking multipath data for {policy} policy...")
    multipath_data_found = False
    for router in routers:
        if hasattr(router, "save_multipath_performance"):
            if hasattr(router, 'multipath_performance_data') and router.multipath_performance_data:
                router.save_multipath_performance(f"multipath_performance_{policy}.csv")
                print(f"[DEBUG] ✓ Saved multipath CSV with {len(router.multipath_performance_data)} records")
                multipath_data_found = True
            else:
                print(f"[DEBUG] Router {router.name} has no multipath data")

    if not multipath_data_found:
        print(f"[DEBUG] ⚠ No multipath data collected - multipath mode may not have been triggered")
        print(f"[DEBUG] Check that Router.use_multipath = True and receive_interest_multipath was called")
...
def main():
    print("=" * 80)
    print(" MULTI-PATH CHUNKED DATA TRANSMISSION SIMULATION")
    print(" WITH ALGORITHM 1 - GOODPUT-BASED PATH SELECTION")
    print("=" * 80)
    print(" ✓ Data divided into 4 chunks before transmission")
    print(" ✓ Multiple network paths for redundancy and performance")
    print(" ✓ Goodput-based chunk allocation across paths")
    print(" ✓ Chunks cached and forwarded individually by routers")
    print("=" * 80)
    ...
    print(f" Publishers: {len(publishers)} (cats, dogs)")
    ...
    for router in routers:
        ...
        for dest, next_hop in router.fib.items():
            if isinstance(next_hop, Router):
                unique_next_hops.add(next_hop)
        path_count = len(unique_next_hops)
        total_paths += path_count
        max_paths = max(max_paths, path_count)

    avg_paths = total_paths / len(routers) if routers else 0
    print(f" Average paths per router: {avg_paths:.1f}")
    print(f" Maximum paths from any router: {max_paths}")
    print(f" Multi-path routing: {'ENABLED ✓' if avg_paths > 1 else 'LIMITED ⚠'}")
    print("=" * 80)
    ...
    for policy in policies:
        ...
        print(f"\n🔧 Running multi-path chunked transmission for {policy} policy...")
        print(f"   Algorithm 1: Goodput-based chunk distribution")
