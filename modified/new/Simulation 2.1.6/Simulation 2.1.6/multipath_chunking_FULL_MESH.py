...
class Router(Node):
    ...
    def _discover_paths_to_publisher(self, content_name):
        """
        Discover multiple distinct paths to publisher using FULL MESH topology.
        Returns ALL available next-hop routers for true multi-path transmission.
        """
        paths = []

        # Method 1: Use stored connections (MAIN method for mesh topology)
        if hasattr(self, 'connections') and self.connections:
            # Add ALL connected routers as potential paths
            for next_hop in self.connections:
                if isinstance(next_hop, Router):
                    paths.append(next_hop)

            self.log_event(f"[MULTIPATH] Found {len(paths)} paths via connections attribute")

        # Method 2: Extract ALL unique routers from FIB
        # This catches any additional paths not in connections
        unique_next_hops = set()
        for dest, next_hop in self.fib.items():
            if isinstance(next_hop, Router) and next_hop not in paths:
                unique_next_hops.add(next_hop)

        # Add FIB-discovered paths
        for router in unique_next_hops:
            if router not in paths:
                paths.append(router)

        if unique_next_hops:
            self.log_event(f"[MULTIPATH] Found {len(unique_next_hops)} additional paths via FIB")

        # Return ALL paths (don't limit to 3 or 5 - use all available!)
        # This is critical for true multi-path operation
        if len(paths) > 1:
            self.log_event(f"[MULTIPATH] ✓ Discovered {len(paths)} DISTINCT paths for {content_name}")
            for i, path_router in enumerate(paths):
                self.log_event(f"[MULTIPATH]   Path {i+1}: via {path_router.name}")
        elif len(paths) == 1:
            self.log_event(f"[MULTIPATH] ⚠ Only 1 path found (limited multi-path)")
        else:
            self.log_event(f"[MULTIPATH] ✗ No paths found - check network topology!")

        return paths  # Return ALL paths, not paths[:5]
...
def setup_network():
    """
    Set up a FULLY MESHED multi-path network topology.
    Creates multiple distinct routes between ANY two routers in the network.
    Each router can reach publishers via MULTIPLE different paths.
    """
    ...
    if num_routers < 6:
        print(f"⚠ Warning: {num_routers} routers may limit path diversity.")
    ...
    print(f"\n📡 Creating FULL MESH multi-path network topology...")
    print(f"   Routers: {num_routers}")
    print(f"   Publishers: 2")
    print(f"   Subscribers: {num_subscribers}")

    # ==================================================================
    # CRITICAL FIX: CREATE MULTIPLE PATHS BY MESHING ROUTERS
    # ==================================================================
    # Problem: Previous implementation only created linear paths
    # Solution: Each router connects to MULTIPLE other routers

    # Strategy 1: Divide routers into groups for structured mesh
    group_size = max(2, num_routers // 3)

    # Strategy 2: Each router connects to several other routers
    # This creates a highly connected graph with many alternative paths

    print(f"\n   Creating mesh connections between routers...")
    connection_count = 0

    # For each router, connect it to multiple other routers
    for i, router in enumerate(routers):
        # List to track this router's connections
        router_connections = []

        # Connect to next router (creates ring)
        next_idx = (i + 1) % len(routers)
        router_connections.append(routers[next_idx])

        # Connect to previous router (bidirectional ring)
        prev_idx = (i - 1) % len(routers)
        if routers[prev_idx] not in router_connections:
            router_connections.append(routers[prev_idx])

        # Connect to router 2 steps ahead (double ring)
        skip2_idx = (i + 2) % len(routers)
        if routers[skip2_idx] not in router_connections:
            router_connections.append(routers[skip2_idx])

        # Connect to router 2 steps behind
        skip2_back_idx = (i - 2) % len(routers)
        if routers[skip2_back_idx] not in router_connections:
            router_connections.append(routers[skip2_back_idx])

        # Connect to router at opposite side of network (diameter path)
        opposite_idx = (i + num_routers // 2) % len(routers)
        if routers[opposite_idx] not in router_connections and opposite_idx != i:
            router_connections.append(routers[opposite_idx])

        # Add these connections to FIB for ALL content
        for next_hop in router_connections:
            for j in range(1, 51):
                # Store multiple next-hops in FIB
                # Note: This creates multiple FIB entries for same content
                router.fib[f"cat_image{j}.jpg_via_{next_hop.name}"] = next_hop
                router.fib[f"dog_image{j}.jpg_via_{next_hop.name}"] = next_hop
            connection_count += 1

        # Store connections for path discovery
        router.connections = router_connections

        print(f"   Router{i+1}: {len(router_connections)} connections -> {[r.name for r in router_connections]}")

    # Ensure last few routers connect to publishers
    # Multiple routers can reach publishers (not just the last one)
    num_publisher_routers = min(3, num_routers)  # At least 3 routers connect to publishers

    for i in range(num_routers - num_publisher_routers, num_routers):
        router = routers[i]
        router.fib.update({f"cat_image{j}.jpg": publisher1 for j in range(1, 51)})
        router.fib.update({f"dog_image{j}.jpg": publisher2 for j in range(1, 51)})
        print(f"   {router.name} -> Publishers (direct)")

    # CRITICAL: Also store standard FIB entries (without _via_ suffix) for backward compatibility
    # Use a round-robin approach to distribute across connections
    for router in routers:
        if hasattr(router, 'connections') and router.connections:
            for j in range(1, 51):
                # Assign to one of the connections (will vary by content)
                conn_idx = j % len(router.connections)
                router.fib[f"cat_image{j}.jpg"] = router.connections[conn_idx]
                router.fib[f"dog_image{j}.jpg"] = router.connections[conn_idx]

    print(f"\n   ✓ Created {connection_count} router-to-router connections")
    print(f"   ✓ Each router has {connection_count // num_routers} average connections")
    ...
    print("\n✅ FULL MESH multi-path network created and saved.")
    return routers, publishers, subscribers
