...
class Router(Node):
    ...
    def _discover_paths_to_publisher(self, content_name):
        ...
        Returns ALL available next-hop routers AND can reach publishers directly.
        """
        paths = []

        # Method 1: Router-to-router paths
        if hasattr(self, 'connections') and self.connections:
            for next_hop in self.connections:
                if isinstance(next_hop, Router):
                    paths.append(next_hop)

            self.log_event(f"[MULTIPATH] Found {len(paths)} router paths via connections")

        # Method 2: Direct publisher connections
        if hasattr(self, 'publisher_connections') and self.publisher_connections:
            for publisher in self.publisher_connections:
                if isinstance(publisher, Publisher):
                    paths.append(publisher)

            self.log_event(f"[MULTIPATH] Found {len(self.publisher_connections)} direct publisher paths")

        # Method 3: Extract from FIB
        unique_next_hops = set()
        for dest, next_hop in self.fib.items():
            if isinstance(next_hop, Router):
                if next_hop not in paths:
                    unique_next_hops.add(next_hop)
            elif isinstance(next_hop, Publisher):
                if next_hop not in paths:
                    unique_next_hops.add(next_hop)

        for hop in unique_next_hops:
            if hop not in paths:
                paths.append(hop)
        ...
        # Log results
        if len(paths) > 1:
            num_router_paths = len([p for p in paths if isinstance(p, Router)])
            num_pub_paths = len([p for p in paths if isinstance(p, Publisher)])
            self.log_event(f"[MULTIPATH] ✓ Discovered {len(paths)} DISTINCT paths ({num_router_paths} router + {num_pub_paths} publisher)")

            for i, path in enumerate(paths):
                if isinstance(path, Router):
                    self.log_event(f"[MULTIPATH]   Path {i+1}: via {path.name} (router)")
                elif isinstance(path, Publisher):
                    self.log_event(f"[MULTIPATH]   Path {i+1}: via {path.name} (publisher)")
        elif len(paths) == 1:
...
def setup_network():
    """
    Set up a FULLY MESHED multi-path network topology WITH PUBLISHER CONNECTIONS.
    Creates multiple distinct routes between ANY two routers AND publishers.
    Each router connects to other routers AND to publishers for multiple end-to-end paths.
    """
    ...
    print(f"\n📡 Creating FULL MESH multi-path network topology WITH PUBLISHER CONNECTIONS...")
    ...
    for i, router in enumerate(routers):
        ...
        # Add connections to FIB (for router-to-router paths)
    ...
    # ==================================================================
    # DIRECT PUBLISHER CONNECTIONS FROM ALL ROUTERS
    # ==================================================================
    # CRITICAL FIX: ALL routers can reach BOTH publishers directly
    # This creates multiple end-to-end paths through the publisher
    print(f"\n   🌐 Connecting ALL routers to BOTH publishers...")

    for router in routers:
        # Each router can reach both publishers
        # This is added to FIB as well
        for j in range(1, 51):
            router.fib[f"cat_image{j}.jpg"] = publisher1
            router.fib[f"dog_image{j}.jpg"] = publisher2

        # Also store publishers in connections for algorithm
        if not hasattr(router, 'publisher_connections'):
            router.publisher_connections = []

        router.publisher_connections.append(publisher1)
        router.publisher_connections.append(publisher2)

        print(f"   {router.name} -> [Publisher1, Publisher2] (direct)")

    # Also add router-to-publisher paths (routers can reach publishers via other routers)
    # This creates: Subscriber -> Router -> Router -> Router -> Publisher (multiple variations)
    for i, router in enumerate(routers):
        # Last router has direct connection to publishers
        if i >= num_routers - 3:  # Last 3 routers
            # Already connected above
            pass
        else:
            # Middle routers can reach publishers via next router
            next_idx = (i + 1) % len(routers)
            next_router = routers[next_idx]

            # Path: this router -> next router -> publisher
            # But next_router already connects to publisher, so this is implicit
            # (algorithm will discover it through router.connections -> FIB lookup)

    print(f"\n   ✓ Created {connection_count} router-to-router connections")
    print(f"   ✓ ALL {num_routers} routers connected to BOTH publishers")
    print(f"   ✓ Multiple end-to-end paths available:")
    print(f"      - Subscriber -> Router -> [multiple paths] -> Publisher")
    print(f"      - Multiple router-to-router intermediate hops possible")

    # Save the new network setup
    save_network(routers, publishers, subscribers)

    print("\n✅ FULL MESH network with PUBLISHER CONNECTIONS created and saved.")
    return routers, publishers, subscribers
