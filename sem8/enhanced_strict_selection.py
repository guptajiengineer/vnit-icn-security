...
"""
Multipath Content Delivery Network Simulator - ENHANCED VERSION
WITH STRICT PATH SELECTION CRITERIA:
- Algorithm 1: Multipath Exploration using Node Weights
- Algorithm 2: STRICT Multipath Selection:
  * Only completely disjoint paths (NO shared nodes)
  * Path weight threshold filtering
  * Node weight filtering (discard low-weight nodes)
  * Node weight = normalized product of node resources
  * Modified path weight formula for better stability
...
class Router(BaseNode):
    ...
    def calculate_modified_path_weight(self, entry, all_entries):
        ...
        p_t(a_k, s_k) = (d_t × (1 - l_t)) / (1 + q_t + o_t)
    ...
    def multipath_selection(self, content_name, 
        ...
        for entry in above_threshold:
            path_str = ' → '.join(entry.node_id_set)
            self.log_event(f"  ✓ {path_str} (weight: {entry.path_weight:.4f})")
        ...
        for i, path_i in enumerate(selected_paths):
            ...
            if not conflicts:
                ...
                self.log_event(f"  ✓ SELECTED: {path_str}")
                ...
                if entry not in final_selected_paths:
                    ...
                    for selected in final_selected_paths:
                        if not self._are_paths_disjoint(entry.node_id_set, selected.node_id_set):
                            self.log_event(f"  ✗ {path_str} conflicts with {' → '.join(selected.node_id_set)}")
                            break
    ...
    def receive_interest(self, interest_packet, subscriber):
        ...
        if interest_packet.name in self.cs:
            ...
            self.log_event(f"✓ Cache HIT: Serving via {len(selected_paths)} disjoint paths\n")
            ...
            if next_hop and next_hop not in interest_packet.visited:
                if isinstance(next_hop, Router):
                    ...
                    if data_packet:
                        ...
                        subscriber.receive_data(data_packet)
                        self.log_event(f"\n✓ Content delivered via {len(selected_paths)} disjoint paths\n")
                        return
