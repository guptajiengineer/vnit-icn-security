...
"""
Multipath Content Delivery Network Simulator - IMPROVED VERSION v2.0

FIXES IMPLEMENTED:
1. Fixed multipath selection algorithm (proper disjoint path filtering)
2. Added path display with selected routes
3. Track hops per data chunk transmission
4. Save hop statistics per iteration (average hops)
5. Calculate hop distance properly for multipath transmission
"""
...
# ===================== ALGORITHM CONSTANTS =====================
PATH_EXPLORATION_TIMEOUT = 10
PATH_WEIGHT_THRESHOLD = 0.4
NODE_WEIGHT_THRESHOLD = 0.3
...
class PathExplorationEntry:
	...
	def __init__(self, name, node_id_set, path_weight, lifetime):
		...
		self.node_weights = {}
		self.hop_count = len(node_id_set) - 1  # Number of hops in path
...
class PathTableEntry:
	...
	def __init__(self, name, node_id_set, lifetime):
		...
		self.neighbors = []
		self.node_weights = {}
		self.hop_count = len(node_id_set) - 1

	def is_expired(self):
		return datetime.datetime.now() > self.creation_time + datetime.timedelta(seconds=self.lifetime)

class EdgeLink:
	"""Represents a link between two nodes with dynamic metrics"""
...
class RandomForestCachingPolicy:
	...
	def extract_features(self, content_name, request_count, popularity, response_time, packet_loss):
		return np.array([request_count, popularity, response_time, packet_loss]).reshape(1, -1)
...
# ===================== ROUTER CLASS WITH IMPROVED ALGORITHMS =====================

class Router(BaseNode):
	"""Router implementing Algorithms 1 & 2 with proper path selection"""
	...
	def __init__(self, name, alpha=0.9):
		...
		self.storage_capacity = random.uniform(0.4, 0.9)
		self.bandwidth_available = random.uniform(0.6, 1.0)
		self.node_weight = self._calculate_node_weight()
		self.resource_threshold = NODE_WEIGHT_THRESHOLD
		...
		# TRACKING METRICS
		self.iteration_hops = []  # List of hops per chunk per iteration
		self.selected_paths_log = []  # Log of selected paths
	...
	def reset(self):
		"""Reset router state for new iteration"""
		self.cache_hits = 0
	...
	def update_node_weight(self):
		"""Update node weight dynamically"""
		...
		for provider in provider_nodes:
			...
			if exploration_entry:
				...
				self.log_event(
					...
					f"({exploration_entry.hop_count} hops)"
				)
	...
	def _explore_path_to_provider(self, content_name, provider, node_id_set, provider_pos, requester_pos):
		"""Explore single path to provider"""
		...
		exploration_entry.hop_count = len(node_id_set) - 1

		return exploration_entry

	# ============ ALGORITHM 2: STRICT MULTIPATH SELECTION (FIXED) ============

	def calculate_modified_path_weight(self, entry, all_entries):
		"""Calculate path weight using modified formula"""
		...
		q_t = entry.pending_requests / denominator if denominator > 0 else 0.0

		denominator = max(1, max_loss) + min_loss
		l_t = entry.packet_loss_rate / denominator if denominator > 0 else 0.0

		denominator = max(1, max_time) + min_time
		o_t = entry.response_time / denominator if denominator > 0 else 0.5
	...
	def multipath_selection(self, content_name,
		...
		"""Algorithm 2: IMPROVED Multipath Selection with proper disjoint filtering"""
		...
		# STEP 1: Calculate path weights
		self.log_event(f"\n========== MULTIPATH SELECTION FOR '{content_name}' ==========")
		self.log_event(f"Total exploration entries: {len(relevant_entries)}")
		self.log_event(f"Path weight threshold: {path_weight_threshold:.2%}")
		...
		self.log_event(f"\n[STEP 1-2] Paths above weight threshold ({path_weight_threshold:.2%}):")
		for i, entry in enumerate(above_threshold, 1):
			path_str = ' -> '.join(entry.node_id_set)
			self.log_event(f"  {i}. {path_str}")
			self.log_event(f"     Weight: {entry.path_weight:.4f}, Hops: {entry.hop_count}")

		if not above_threshold:
			self.log_event("\nNo paths above threshold - cannot select multipaths")
			return []

		# STEP 3: Group by provider and select best path per provider
		self.log_event(f"\n[STEP 3] Select best path per provider:")
		...
		for entry in sorted(above_threshold, key=lambda x: x.path_weight, reverse=True):
			...
			if provider_id not in selected_by_provider:
				...
				path_str = ' -> '.join(entry.node_id_set)
				self.log_event(f"  Provider '{provider_id}': {path_str} (weight: {entry.path_weight:.4f})")

		selected_paths = list(selected_by_provider.values())

		# STEP 4: STRICT DISJOINT PATH FILTERING (KEY FIX)
		# For each pair of paths, if they overlap, remove the one with LOWER weight
		self.log_event(f"\n[STEP 4] Enforce DISJOINT paths (remove overlapping ones):")
		...
		for i, path_i in enumerate(selected_paths):
			...
			is_disjoint_from_selected = True
			
			# Check against all already selected paths
			for path_selected in final_selected_paths:
				if not self._are_paths_disjoint(path_i.node_id_set, path_selected.node_id_set):
					is_disjoint_from_selected = False
					break
			
			if is_disjoint_from_selected:
				# Check if this path overlaps with any remaining path (lower weight should be removed)
				paths_to_check = [selected_paths[j] for j in range(i+1, len(selected_paths)) if j not in used_indices]
				
				overlapping_indices = []
				for j, path_j in enumerate(paths_to_check):
					if not self._are_paths_disjoint(path_i.node_id_set, path_j.node_id_set):
						overlapping_indices.append(selected_paths.index(path_j))
				
				# Mark overlapping paths with lower weights for removal
				for idx in overlapping_indices:
					if selected_paths[idx].path_weight < path_i.path_weight:
						used_indices.add(idx)
				
				final_selected_paths.append(path_i)
				path_str = ' -> '.join(path_i.node_id_set)
				self.log_event(f"  [SELECTED] {path_str} (hops: {path_i.hop_count})")
			else:
				path_str = ' -> '.join(path_i.node_id_set)
				self.log_event(f"  [REJECTED] {path_str} (overlaps with selected paths)")

		# Create path table entries for final selected paths
		self.log_event(f"\n========== FINAL SELECTION SUMMARY ==========")
		self.log_event(f"Paths explored: {len(weighted_entries)}")
		self.log_event(f"Paths above threshold: {len(above_threshold)}")
		self.log_event(f"Paths selected (disjoint): {len(final_selected_paths)}")
		...
		for i, path_entry_src in enumerate(final_selected_paths, 1):
			...
			path_entry.hop_count = path_entry_src.hop_count
			self.path_table.append(path_entry)
			
			# Log selected path details
			path_str = ' -> '.join(path_entry.node_id_set)
			self.log_event(f"\n[SELECTED PATH {i}]")
			self.log_event(f"  Route: {path_str}")
			self.log_event(f"  Weight: {path_entry.path_weight:.4f}")
			self.log_event(f"  Hops: {path_entry.hop_count}")
			
			# Store in log
			self.selected_paths_log.append({
				'path': path_str,
				'hops': path_entry.hop_count,
				'weight': path_entry.path_weight
			})
	...
	def split_data_into_parts(self, content_name, content_data, part_size=DATA_PART_SIZE):
		"""Split content into parts for transmission"""
	...
	def transmit_data_over_multipaths(self, content_name, content_data, selected_paths):
		"""Transmit data parts over selected DISJOINT multipaths and track hops"""
		...
		for path_idx, path in enumerate(selected_paths, 1):
			...
			self.log_event(f"  Path {path_idx}: {path_str} (hops: {path.hop_count})")
			...
			for part_idx in range(start_part, end_part):
				if part_idx < len(data_parts):
					...
					part.hops_used = path.hop_count  # TRACK HOPS PER CHUNK
					
					transmission_results.append(part)
					self.iteration_hops.append(path.hop_count)  # Save for averaging
					
					self.log_event(f"    Part {part.part_number}/{part.total_parts} -> {path.hop_count} hops")
	...
	def receive_interest(self, interest_packet, subscriber):
		...
		self.log_event(f"\n>>> Interest: '{interest_packet.name}' from {subscriber.name}")

		if self.name in interest_packet.visited:
			self.log_event(f"Loop detected - dropping")
			return
		...
		if interest_packet.name in self.cs:
			...
			self.log_event(f"[CACHE HIT] Served via {len(selected_paths)} disjoint paths\n")
			subscriber.receive_data(data_packet)
			return

		# Cache miss
		self.publisher_hits += 1
		self.log_event(f"[CACHE MISS] Fetching from network")
			...
			if next_hop and next_hop not in interest_packet.visited:
				if isinstance(next_hop, Router):
					...
					if data_packet:
						...
						subscriber.receive_data(data_packet)
						self.log_event(f"[CONTENT DELIVERED] via {len(selected_paths)} disjoint paths\n")
						return

	def receive_data(self, data_packet):
		"""Handle incoming data packets with caching"""
	...
	def log_event(self, message):
		"""Write log files with UTF-8 encoding"""
...
def plot_network_with_multipaths(routers, publishers, subscribers):
	...
	plt.title(f"Multipath Network with STRICT Selection (IMPROVED v2.0)\n(Proper Disjoint Paths, Weight-based, Hop Tracking)",
...
def run_multipath_simulation(routers, publishers, subscribers, iterations):
	...
	simulation_data = []
	hops_per_iteration = []  # Track average hops per iteration
	...
	for iteration in range(iterations):
		...
		for router in routers:
			...
			router.iteration_hops = []  # Reset for this iteration
		...
		# CALCULATE AVERAGE HOPS PER CHUNK IN THIS ITERATION
		all_iteration_hops = []
		for router in routers:
			all_iteration_hops.extend(router.iteration_hops)
		
		avg_hops = np.mean(all_iteration_hops) if all_iteration_hops else 0
		hops_per_iteration.append(avg_hops)
		...
		print(f" Average Hops per Chunk: {avg_hops:.2f}\n")
		...
		simulation_data.append([
			...
			total_paths,
			avg_cache_hit,
			avg_hops,  # ADD HOP COUNT
			'StrictSelection-v3'
		])

	return simulation_data, hops_per_iteration

def save_multipath_results(all_simulation_data, num_routers, num_subscribers, num_publishers, iterations):
	"""Save simulation results with hop statistics"""
	...
	with open(filename, mode='w', newline='', encoding='utf-8') as file:
		...
		writer.writerow([
			...
			"Cache Hit Ratio (%)",
			"Average Hops per Chunk",  # NEW COLUMN
			"Method"
		])
...
def get_user_input():
	...
	print("\n" + "="*70)
	print("MULTIPATH CDN SIMULATOR - IMPROVED v2.0")
	print("="*70 + "\n")
...
def main():
	...
	simulation_data, hops_per_iteration = run_multipath_simulation(routers, publishers, subscribers, iterations)
	...
	print(f"Path Selection: STRICT + PROPER DISJOINT FILTERING (v3.0)")
	print(f"Average Hops Across All Iterations: {np.mean(hops_per_iteration):.2f}")
	print(f"\nOutput:")
	print(f" • Network: output/network_topology_strict.png")
	print(f" • Results: Simulation_Results/StrictSelection/")
	print(f" • Logs: Logs/log_Router*.txt (detailed path selection + hops)")
