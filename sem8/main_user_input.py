...
With USER INPUT FOR: Routers, Publishers, Subscribers, Iterations
...
def run_multipath_simulation(routers, publishers, subscribers, iterations):
    """Run simulation with variable inputs and RandomForest policy"""
    for router in routers:
...
def save_multipath_results(all_simulation_data, num_routers, num_subscribers, num_publishers, iterations):
    ...
    filename = f'Simulation_Results/Multipath/RandomForest_{num_routers}r_{num_publishers}p_{num_subscribers}s_{iterations}i_results.csv'
...
def plot_multipath_performance(all_results, num_routers, num_subscribers, num_publishers):
...
def get_user_input():
    ...
    print("\n" + "="*70)
    print("MULTIPATH CDN SIMULATOR - NETWORK CONFIGURATION")
    print("="*70 + "\n")
...
def main():
    ...
    plot_multipath_performance(simulation_data, num_routers, num_subscribers, num_publishers)
