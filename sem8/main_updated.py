...
With VARIABLE SUBSCRIBERS, ROUTERS, AND ITERATIONS
RANDOMFOREST CACHING POLICY ONLY
"""
...
class RandomForestCachingPolicy:
    ...
    def predict_cache_importance(self, content_name, request_count, popularity, response_time, packet_loss):
        ...
        if request_count < 5:  # Need minimum training data
...
class Router(Node):
    ...
    def receive_data(self, data_packet):
        ...
        if len(self.cs) >= Router.CACHE_LIMIT:
            ...
            # Use RandomForest to decide what to evict
            evict_content = None
            min_score = float('inf')
...
def setup_network_with_multipaths(num_routers=5, num_publishers=3, num_subscribers=1, use_saved=True):
    ...
    print(f"Routers: {num_routers} (VARIABLE INPUT)")
    print(f"Publishers: {num_publishers}")
    print(f"Subscribers: {num_subscribers} (VARIABLE INPUT)")
    print(f"================================\n")
    ...
    # Create MULTIPLE subscribers (VARIABLE)
...
def run_multipath_simulation(routers, publishers, subscribers, iterations):
    """Run simulation with variable subscribers and RandomForest policy"""
    for router in routers:
    ...
    print(f"Running {iterations} iterations with RandomForest caching policy...")
    simulation_data = []
    active_prob = 0.95
...
def save_multipath_results(all_simulation_data, num_subscribers, num_publishers, num_routers):
    """Save simulation results"""
    os.makedirs('Simulation_Results/Multipath', exist_ok=True)
    filename = f'Simulation_Results/Multipath/RandomForest_{num_routers}router_{num_subscribers}sub_{num_publishers}pub_results.csv'
...
def plot_multipath_performance(all_results, num_subscribers, num_routers):
    ...
    axs[0, 0].plot(df['Iteration'], df['Cache Hit Ratio (%)'], color='navy', marker='o', markersize=6, linewidth=2)
    axs[0, 0].set_title('Cache Hit Ratio over Iterations (RandomForest)', fontsize=12, fontweight='bold')
    axs[0, 0].set_xlabel('Iteration')
    axs[0, 0].set_ylabel('Cache Hit Ratio (%)')
    axs[0, 0].grid(True, linestyle='--', alpha=0.5)

    axs[0, 1].plot(df['Iteration'], df['Latency'], color='darkgreen', marker='o', markersize=6, linewidth=2)
    axs[0, 1].set_title('Latency over Iterations', fontsize=12, fontweight='bold')
    axs[0, 1].set_xlabel('Iteration')
    axs[0, 1].set_ylabel('Latency')
    axs[0, 1].grid(True, linestyle='--', alpha=0.5)

    axs[1, 0].plot(df['Iteration'], df['Hop Reduction Ratio'], color='darkorange', marker='o', markersize=6, linewidth=2)
    ...
    axs[1, 1].plot(df['Iteration'], df['Active Paths'], color='indigo', marker='o', markersize=6, linewidth=2)
    ...
    plt.suptitle(f'RandomForest Caching Policy Performance\n({num_routers} Routers, {num_subscribers} Subscribers)',
        fontsize=14, fontweight='bold')
    plt.tight_layout()

    os.makedirs('output', exist_ok=True)
    plt.savefig('output/randomforest_performance.png', dpi=300, bbox_inches='tight')
    print("Performance plot saved as 'output/randomforest_performance.png'\n")
    plt.close()

def main():
    print("\n" + "="*70)
    print("MULTIPATH CONTENT DELIVERY NETWORK SIMULATOR")
    print("VARIABLE ROUTERS, SUBSCRIBERS, AND ITERATIONS")
    print("RANDOMFOREST CACHING POLICY")
    print("="*70 + "\n")

    # Get user input for routers
    while True:
        try:
    ...
    # Get user input for subscribers
    while True:
        try:
    ...
    # Run simulation
    print(f"Running simulation for {iterations} iterations with {num_subscribers} subscribers and {num_routers} routers...\n")
    simulation_data = run_multipath_simulation(routers, publishers, subscribers, iterations)

    # Save results
    save_multipath_results(simulation_data, num_subscribers, num_publishers, num_routers)

    # Plot performance
    print("Generating performance plots...\n")
    plot_multipath_performance(simulation_data, num_subscribers, num_routers)
    ...
    print(f" Publishers: {num_publishers}")
    print(f" Subscribers: {num_subscribers} (VARIABLE INPUT)")
    print(f" Iterations: {iterations} (VARIABLE INPUT)")
    print(f" Total Content Items: {sum(len(p.contents) for p in publishers)}")
    print(f" Caching Policy: RandomForest (ONLY)")
    print(f"\nOutput Files:")
    print(f" • Network topology: output/network_topology_multipath.png")
    print(f" • Performance plots: output/randomforest_performance.png")
