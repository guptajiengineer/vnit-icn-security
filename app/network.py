...
from models import BaseTopology, ContentSpec, PathRecord, ResourceBudget, SimNode
from paths import bfs_shortest_path
...
def  make_router_resources(rng: random.Random) -> Dict[str, ResourceBudget]:
    profiles = {
        "cpu": (30.0, 2.1, 0.08),
...
def  sample_router(node_id: str, x: float, y: float, rng: random.Random) -> SimNode:
    return SimNode(
        node_id=node_id,
        x=x,
        y=y,
        ...
        resources=make_router_resources(rng),
    )


def  shape_metrics_by_distance(
...
def  connect(adjacency: Dict[str, Set[str]], a: str, b: str) :
    adjacency[a].add(b)
    adjacency[b].add(a)


def  ensure_connected(nodes: Dict[str, SimNode], adjacency: Dict[str, Set[str]]) :
    ...
    for  _ in range(len(nodes)):
        ...
        connect(adjacency, best_pair[0], best_pair[1])


def  bfs_hops(adjacency: Dict[str, Set[str]], start: str) -> Dict[str, int]:
    queue: deque[str] = deque([start])
    hops = {start: 0}
...
def  pick_publisher_candidates(
    ...
    edge_hop_maps = [bfs_hops(adjacency, edge_id) for edge_id in edge_node_ids]
...
def build_base_topology(
    ...
    edge_positions = [
        ...
        (mid, 12.0),
        (mid, max(12.0, area_size - 12.0)),
        (max(12.0, area_size - 12.0), 12.0),
        (max(12.0, area_size - 12.0), mid),
    ...
    for anchor_index, (x, y) in enumerate(anchor_positions):
        ...
        node =  sample_router(node_id, x, y, rng)
        nodes[node_id] = node
    ...
    while len(service_nodes) < service_count:
        ...
        if  too_close(x, y, positions, min_spacing):
            continue
    ...
    shape_metrics_by_distance(edge_nodes, service_nodes, rng)
    ...
    for idx, node_a_id in enumerate(all_node_ids):
        ...
        for node_b_id in all_node_ids[idx + 1 :]:
            ...
            if node_a.distance_to(node_b) <= communication_range:
                 connect(adjacency, node_a_id, node_b_id)

    ensure_connected(nodes, adjacency)

    publisher_candidates =  pick_publisher_candidates(
        nodes,
        adjacency,
...
def  content_availability(selected_paths: Sequence[PathRecord]) -> float:
...
def  caching_weight(
    ...
    availability =  content_availability(selected_paths)
...
def choose_cache_node(
    ...
    if not selected_paths:
        return None
    if  caching_weight(content, selected_paths, round_index=round_index) <= 0.0:
        return None
...
def get_user_nodes(
    ...
    """Pick subscriber-facing edge nodes for the paper-style ``Nu`` sweep.

    The paper models users ``Ui`` as attached to edge nodes ``ENi``. We
    therefore choose only topology edge nodes here and order them from farther
    to nearer provider distance so that increasing ``Nu`` adds progressively
    closer user access points.
    """
    candidate_ids = [
        node_id
        for node_id in base.edge_node_ids
        if node_id not in active_publishers
    ]
    ...
    scored = []
    for node_id in candidate_ids:
        hops =  nearest_provider_hops(base, node_id, active_publishers)
        degree = len(base.adjacency[node_id])
        scored.append((hops, degree, node_id))

    scored.sort(key=lambda item: (-item[0], item[1], item[2]))
    # return [node_id for _, _, node_id in scored[:max_candidates]]
    return candidate_ids


def expand_user_edge_nodes(
    ordered_edge_nodes: Sequence[str],
    num_users: int,
) -> List[str]:
    if num_users <= 0 or not ordered_edge_nodes:
        return []

    expanded: List[str] = []
    while len(expanded) < num_users:
        remaining = num_users - len(expanded)
        expanded.extend(ordered_edge_nodes[:remaining])
    return expanded[:num_users]
